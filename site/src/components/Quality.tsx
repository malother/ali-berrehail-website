      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="mx-auto max-w-2xl text-center">
          ...
          <Reveal delay={1}>
            <h2 className="mt-5 font-display text-4xl font-extrabold leading-tight text-white sm:text-5xl">
              {d.quality.t1} <span className="text-elec-metal">{d.quality.t2}</span>
            </h2>
          </Reveal>
          <Reveal delay={2}>
        ...
        <Parallax range={24}>
          <div className="mt-14 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
              ...
              return (
                <Reveal key={it.title} delay={(i % 2 ? 2 : 1) as 1 | 2}>
                  <article className="glass group relative h-full overflow-hidden rounded-2xl p-6 transition-all duration-300 hover:-translate-y-0.5 hover:border-elec-500/30">
                    <span
                      className={`absolute inset-x-8 top-0 h-px bg-gradient-to-r from-transparent ${crimson ? 'via-crimson-500/60' : 'via-elec-500/60'} to-transparent`}
                      aria-hidden="true"
                    />
                    <span
            ...
            <Reveal delay={2}>
              <article className="glass relative h-full overflow-hidden rounded-2xl border-l-2 border-l-elec-500 p-6">
